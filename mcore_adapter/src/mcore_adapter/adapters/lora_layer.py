...
# tp_group as an explicit constructor arg was added to megatron-core's TE linear extensions in a
# later release than what may be installed here; older releases infer the group from global mpu state.
_TE_LINEAR_ACCEPTS_TP_GROUP = "tp_group" in inspect.signature(TERowParallelLinear.__init__).parameters
...
class LoraParallelLinear(MegatronModule, LoraLayer):
    ...
    def _create_lora_layers(self, r, lora_bias, **kwargs):
        """Create LoRA A and B layers. To be implemented by subclasses."""
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        if lora_dropout > 0.0:
            lora_dropout_layer = nn.Dropout(lora_dropout)
        else:
        ...
        # tp_group is only accepted by newer megatron-core TE extension signatures; this installed
        # version infers the tensor-parallel group from global mpu state instead.
        if _TE_LINEAR_ACCEPTS_TP_GROUP:
        ...
        # Disable sequence parallel for LoRA layers
...
class LoraRowParallelLinear(LoraParallelLinear):
...
class LoraColumnParallelLinear(LoraParallelLinear):
...
def patch_TEGroupedLinear():
    ...
    setattr(TEGroupedLinear, "sharded_state_dict", sharded_state_dict)
